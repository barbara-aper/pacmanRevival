...
{
    tFantasma **fantasmas = (tFantasma **)malloc(6 * sizeof(tFantasma));
    ...
    fantasmas[4] = AlocaFantasma(ObtemPosicaoItemMapa(mapa, 'H'), 'H');
    fantasmas[5] = AlocaFantasma(ObtemPosicaoItemMapa(mapa, 'V'), 'V');
...
void MoveFantasma(tFantasma *fantasma, tMapa *mapa)
{
    ...
    if (ObtemLinhaPosicao(fantasma->posicaoAtual) == 1 && fantasma->tipo == 'V')
    {
        andaLinha = (ObtemNumeroLinhasMapa(mapa) - 3);
    }
    else if (ObtemColunaPosicao(fantasma->posicaoAtual) == ObtemNumeroColunasMapa(mapa) - 2 && fantasma->tipo == 'H')
    {
        andaColuna = -(ObtemNumeroColunasMapa(mapa) - 3);
    }
    else if (fantasma->direcao == MOV_BAIXO)
        andaLinha = 1;
    ...
    if (EncontrouParedeMapa(mapa, proxima) && fantasma->tipo != 'H' && fantasma->tipo != 'V')
    {
...
void MoveFantasmas(tFantasma **fantasmas, tMapa *mapa)
{
    ...
    MoveFantasma(fantasmas[4], mapa);
    MoveFantasma(fantasmas[5], mapa);
}
