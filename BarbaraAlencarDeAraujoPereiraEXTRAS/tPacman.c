...
void MovePacman(tPacman *pacman, tMapa *mapa, COMANDO comando)
{
    ...
    if (EncontrouParedeMapa(mapa, pacman->posicaoAtual) && !pacman->andaParede)
    {
        MataPacman(pacman);
        return;
    }
    ...
    if (EncontrouParedeMapa(mapa, proxima))
    {
        ...
        else if (ObtemLinhaPosicao(proxima) == 0 || ObtemLinhaPosicao(proxima) == ObtemNumeroLinhasMapa(mapa) - 1 || 
        ObtemColunaPosicao(proxima) == 0 || ObtemColunaPosicao(proxima) == ObtemNumeroColunasMapa - 1)
        {
            DesalocaPosicao(proxima);
            AtualizaTrilhaPacman(pacman);
            return;
    ...
    else if (ObtemItemMapa(mapa, proxima) == '+')
    {
        AtualizaItemMapa(mapa, proxima, ' ');
        pacman->andaParede = 15;
    }
    else if (ObtemItemMapa(mapa, proxima) == 'x')
    {
        AtualizaItemMapa(mapa, proxima, ' ');
        pacman->matador = 15;
    }
...
int EstaMatando(tPacman *pacman)
{
    return pacman->matador;
}

void MatouFantasma(tPacman *pacman)
{
    pacman->matador = 0;
}
...
void SalvaTrilhaPacman(tPacman *pacman)
{
    ...
    if (!arquivoTrilha)
    {
        printf("Erro ao abrir o arquivo trilha.txt\n");
        return;
    }
